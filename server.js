...
const findJobs = async (query, page, pageSize) => {
...
Ensure the 'source_url' field contains the direct URL to the job posting. Again, your entire response must be ONLY the raw JSON array string.`;
    ...
    try {
       ...
       if (!Array.isArray(parsedData)) throw new Error("API did not return a valid array of jobs.");
...
app.get('/api/jobs', async (req, res) => {
    ...
    try {
        const jobs = await findJobs(q, parseInt(page), parseInt(pageSize));
        res.json(jobs);
    } catch (error) {
